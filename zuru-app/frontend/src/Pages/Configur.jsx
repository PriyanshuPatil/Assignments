...
const handleUpload = async () => {
  ...
  const get_project=async() => {
    try {
      let res=await axios.get(`http://localhost:3400/project/${id}`);
  ...
  const update_project=async(id,link) => {
    if(id!=undefined){
     try {
      let res=await axios.get(`http://localhost:3400/project/${id}?link=${link}`);
    } catch (err) {
      console.log(err.data)
    } 
    }
               ...
               <Box>
                   ...
                   {active  && <Box w={"50px"}
                     ...
                     {!active  && <img
                      ...
                      <Button backgroundColor={'blueviolet'} _hover={{backgroundColor:'black'}} color={'white'} onClick={()=>{if(selectedFile==null){
toast({
  title: "Error",
  ...
  isClosable: true,
                        ...
                        handleUpload();setActive(false)}}}>Upload Image</Button>
                    </Box>
