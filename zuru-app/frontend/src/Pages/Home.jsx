...
import {
  Add_project_axios,
  Get_project_axios,
} from "../Redux/projectReducer/project.action";
import { login, logout } from "../Redux/authReducer/auth.action";
const Home = () => {
  ...
  return (
    <>
      <Box className={style.nav_parent}>
        <Box className={style.nav_logo_div}>
          <img src={"https://i.postimg.cc/nc5LXz1k/logois3.png"} />
        </Box>
      ...
      {projectData.length == 0 && (
        <Box>
          ...
          <Box className={style.metting_image}>
            <img src={"https://i.postimg.cc/mrf7Gsg2/logois2.png"} />
          </Box>
          <Box className={style.metting_text}>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
            eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim
            ad minim veniam, quis nostrud exercitation ullamco labons nisi ut
            aliquip ex ea commodo consequat. Duis aute irure dolor in
            reprehenderit in
          </Box>
          <Box className={style.project_button}>
            <Button
              onClick={onOpen}
              w="100%"
              color={"white"}
        ...
        <Box className={style.project_parent}>
          <Box>
            <Box className={style.project_heading_parent}>
              ...
              <Box className={style.project_heading_button}>
                <Button
                  onClick={onOpen}
                  w="100%"
                  color={"white"}
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent mt="200px">
          ...
          <ModalBody pb={6}>
            <FormControl>
              <FormLabel color={"gray"} fontSize={"13px"}>
                Enter Project Name
              </FormLabel>
              <Input
                onChange={(e) =>
          ...
          <ModalFooter gap={"20px"}>
            ...
            <Button
              ...
              onClick={() => {
                dispatch(
                  Add_project_axios(singleProject, userData.email, toast)
                ),
                  onClose();
              }}
            >
      ...
      {isAuth == false && (
        <Modal isOpen={login_model} onClose={onClose}>
          <ModalOverlay />
          <ModalContent mt="200px">
            <ModalHeader fontWeight={"700"}>Login Form</ModalHeader>
            <ModalCloseButton
              onClick={() => {
                modelStatus(false);
              }}
            ...
            <ModalBody pb={6}>
              <FormControl>
                ...
                <Input
                  onChange={(e) => {
                    setLoginData({ ...login_data, name: e.target.value });
                  }}
                  placeholder="Type Name"
                  fontSize={"15px"}
                />
            ...
            <ModalFooter gap={"20px"}>
              <Button
                ...
                onClick={() => {
                  modelStatus(false);
                  dispatch(login(login_data, toast));
                }}
              >
                Save
              </Button>
